...
1. **[Forecasting]** *FEV-BENCH* is benchmark of 100 forecasting tasks across seven domains.
...
Each lookback window of forecasting benchmark is treated as a partially observed sequence with synthetically introduced missing values.
...
1. Download [TS-ICL](https://huggingface.co/taharnbl/TS-ICL) checkpoint and store locally to `<TSICL_PATH>`.
...
4. Run the desired benchmark using the scripts `run_<benchmark_name>.sh` in `bench/<benchmark_name>`
