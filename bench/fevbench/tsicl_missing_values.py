...
from bench.fevbench.utils.missing_values import apply_pointwise_mask, derive_series_rng
from tsicl.pipeline import TSICL

DEFAULT_MASKING_RATIOS = [0.3, 0.5, 0.7, 0.9, 0]
...
def convert_fev_window_to_list_of_dicts_input(
    ...
    """
    (i) Prepare fev inputs (ii) cut to max_context_len (iii) apply pointwise mask (to target only)
    """
    import fev
    ...
    for idx, target_row in enumerate(target_data):
        ...
        target_tensor_i = target_tensor_i[:,-max_context_len:]
        if masking_ratio > 0.0:
            rng = derive_series_rng(base_seed, task_idx, window_idx, idx)
            list_masked_tensor = [
                torch.Tensor(
                    apply_pointwise_mask(
                        target_tensor_i[channel_idx], masking_ratio, rng
                    )    
                )
                for channel_idx in range(len(target_tensor_i))
            ]
            target_tensor_i = torch.stack(list_masked_tensor, dim=1)
        ...
        if len(dynamic_columns) > 0:
            ...
            entry["past_covariates"] = {
                col: torch.Tensor(rearrange(past_covariate_row[col], "t -> t 1")) for col in dynamic_columns if past_covariate_row[col].dtype.kind in "fuib"
            }

        if len(known_dynamic_columns) > 0:
            future_known_row = future_known_data[idx]
            entry["future_covariates"] = {
                col: torch.Tensor(rearrange(future_known_row[col], "t -> t 1")) for col in known_dynamic_columns if future_known_row[col].dtype.kind in "fuib"
            }
...
def _predict_window_missing(
    ...
    for batch in batchify(inputs, batch_size=batch_size):
        ...
        not_full_nans = np.array(
            [(~torch.Tensor(x["target"]).squeeze(-1).isnan()).sum(0) > 0 for x in batch]
        )
        batch_clean = [x for x in batch if (~torch.Tensor(x["target"]).isnan()).sum(0) > 0]
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if len(batch_q) < len(batch):
                    output = np.nan * torch.ones((len(batch), *batch_q.shape[1:]))
                    output[not_full_nans] = batch_q.clone()
                    batch_q = output.clone()
                assert isinstance(batch_q, torch.Tensor)

                # 
                bs, c, t, q = batch_q.shape
...
def run_tsicl_experiment_missing(
    task,
    model_path: Path | str,
    masking_ratio: float,
    task_idx: int,
    ...
    for window_idx, window in enumerate(task.iter_windows()):
        ...
        quantiles_np, means_np = _predict_window_missing(
            model                 = model,
            inputs                = inputs,
...
def main(cfg : DictConfig):
    ...
    # build output path:
    output_path = Path( this_file_dir ) / "results" / "with_missing_values"
    ...
    # get masking ratios:
    masking_ratios = list(
        OmegaConf.select(cfg, "masking_ratios", default=DEFAULT_MASKING_RATIOS)
    )
    ...
    # set expe parameters:
    use_covariates     = bool(OmegaConf.select(cfg, "use_covariates", default=False))
    ...
    for ratio in masking_ratios:
        ...
        out_csv = output_path / "fevbench_tsicl_missing_{}.csv".format(ratio_pct)
        ...
        for task_idx, task in enumerate(tasks, 1):
            ...
            try:
                predictions_per_window, inference_time, extra_info = run_tsicl_experiment_missing(
                    task                  = task,
                    model_path            = model_path,
